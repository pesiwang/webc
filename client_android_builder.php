...
class ClientAndroidBuilder extends Builder {
	...
	public function build() {
		...
		$content = $smarty->fetch(__DIR__ . '/client/android/WebcObject.tpl');
		$this->saveFile('/WebcObject.java', $content);

		$smarty = $this->getSmarty();
		$smarty->assign('interfaces', $parser->getInterfaces());
		$content = $smarty->fetch(__DIR__ . '/client/android/WebcClient.tpl');
...
try{
	$builder = new ClientAndroidBuilder($argv[1], $argv[2]);
	$builder->build();
}
catch(Exception $e){
